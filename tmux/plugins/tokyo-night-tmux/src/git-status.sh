...
if test "$BRANCH" != ""; then
  if test "$STATUS" = "0"; then
    echo "#[fg=##1ae0d7,bg=#131517,bold]  $RESET$BRANCH "
  else
    echo "#[fg=##1ae0d7,bg=#131517,bold]  $RESET$BRANCH $RESET$STATUS_CHANGED$RESET$STATUS_INSERTIONS$RESET$STATUS_DELETIONS"
  fi
fi
